...
app.post("/login/submit", async (req, res) => {
  ...
  if (userResult) {
    ...
    if (verifyValue) {
      ...
      if (req.session.user.role) return res.redirect("/admin/project");
    } else {
...
app.get("/", async (req, res) => {
  ...
  if (!projList.length && userSession) {
    const initializeProj = await projects.initializeProjects(
      userSession.userId
    );
...
app.get("/admin/project", async (req, res) => {
  ...
  res.render("proj-list", {
    title: title,
    directory: formatProjList,
    userSession: req.session.user || null,
  });
});
...
app.get("/admin/user", async (req, res) => {
  ...
  res.render("user-list", {
    title: "Students in HTTP5214",
    students: formatUserList,
    userSession: req.session.user || null,
  });
});
